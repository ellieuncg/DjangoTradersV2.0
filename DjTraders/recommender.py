...
class ProductRecommender:
    ...
    def _get_frequently_bought_products(self) -> Dict:
        ...
        frequently_bought = OrderDetail.objects.filter(
            order__customer_id=self.customer_id
        ).values(
            'product'
        ).annotate(
            purchase_count=Count('order_detail_id')
    ...
    def _get_category_based_recommendations(self) -> Dict:
        ...
        favorite_categories = OrderDetail.objects.filter(
            order__customer_id=self.customer_id
        ).values(
            'product__category'
        ).annotate(
            category_count=Count('order_detail_id')
        ).order_by('-category_count')
        
        recommendations = {}
        for category in favorite_categories:
            similar_products = Product.objects.filter(
                category_id=category['product__category'],
                status='active'
            ).exclude(
                orderdetails__order__customer_id=self.customer_id
            ).annotate(
                category_score=Value(category['category_count'], output_field=IntegerField())
            )
    ...
    def _get_similar_customer_recommendations(self) -> Dict:
        ...
        customer_categories = OrderDetail.objects.filter(
            order__customer_id=self.customer_id
        ).values_list('product__category', flat=True).distinct()
        
        similar_customers = OrderDetail.objects.filter(
            product__category__in=customer_categories
        ).exclude(
            order__customer_id=self.customer_id
        ).values(
            'order__customer'
        ).annotate(
            common_categories=Count('product__category', distinct=True)
        ).filter(
            common_categories__gte=len(customer_categories) * 0.5
        ).values_list('order__customer_id', flat=True)
        
        similar_purchases = OrderDetail.objects.filter(
            order__customer_id__in=similar_customers
        ).exclude(
            product__orderdetails__order__customer_id=self.customer_id
        ).values(
            'product'
        ).annotate(
            purchase_count=Count('order_detail_id')

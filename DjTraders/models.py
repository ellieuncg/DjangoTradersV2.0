...
class Customer(models.Model):
    ...
    city = models.CharField(
        max_length=100, validators=[no_numbers_validator], blank=True, null=True
    )
    ...
    loyalty_level = models.CharField(
        max_length=10, choices=LOYALTY_LEVELS, null=True, blank=True
    )
    ...
    def get_discount_percentage(self):
        discounts = {self.PLATINUM: 7.5, self.DIAMOND: 5.0, self.GOLD: 2.5}
    ...
    def update_annual_spend(self):
        ...
        annual_total = (
            OrderDetail.objects.filter(
                order__customer=self, order__order_date__gte=one_year_ago
            )
            .annotate(
    ...
    def get_spend_to_next_level(self):
        ...
        current_threshold = thresholds.get(self.loyalty_level)
        if current_threshold:
            return max(0, current_threshold - self.annual_spend)
        return None
